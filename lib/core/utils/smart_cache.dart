...
class CacheEntry {
  ...
  double get lruScore {
    ...
    final ageWeight = math.max(0.1, 1.0 - (ageInMinutes / 1440.0)); // Normalizado por dia
    final accessWeight = math.min(2.0, math.log(accessCount + 1));
    final recencyWeight = math.max(0.1, 1.0 - (minutesSinceLastAccess / 60.0)); // Normalizado por hora
    
    return (ageWeight * 0.3) + (accessWeight * 0.4) + (recencyWeight * 0.2) + (priority * 0.1);
  }
}
...
class SmartCache {
  ...
  Future<void> set(
    ...
    try {
      ...
      AppLogger.debug('Cache SET: $key (${_formatBytes(sizeBytes)})', 'SmartCache');
    } catch (e, stackTrace) {
      AppLogger.error('Erro ao armazenar no cache', 'SmartCache', e, stackTrace);
    }
  }
  ...
  Future<T?> get<T>(String key, {T? defaultValue}) async {
    try {
      ...
      AppLogger.error('Erro ao recuperar do cache', 'SmartCache', e, stackTrace);
      return defaultValue;
    }
  }
  ...
  Future<void> clear() async {
    ...
    _cache.clear();
    _currentSizeBytes = 0;
    AppLogger.info('Cache CLEAR: $entriesCount entradas removidas', 'SmartCache');
  }
  ...
  Future<void> cleanup() async {
    ...
    if (removedCount > 0) {
      AppLogger.info('Cache CLEANUP: $removedCount entradas removidas, '
          '${_formatBytes(freedBytes)} liberados', 'SmartCache');
    }
  }
  ...
  Future<void> optimize() async {
    ...
    AppLogger.info('Cache OPTIMIZE: $toRemoveCount entradas removidas, '
        '${_formatBytes(removedBytes)} liberados', 'SmartCache');
  }
  ...
  Map<String, dynamic> getStats() {
    ...
    return {
      ...
      'average_entry_size_bytes': _cache.isNotEmpty 
  ...
  Future<void> _ensureSpace(int requiredBytes) async {
    ...
    while (_currentSizeBytes + requiredBytes > maxSizeBytes && _cache.isNotEmpty) {
      final entries = _cache.entries.toList();
      entries.sort((a, b) => a.value.lruScore.compareTo(b.value.lruScore));
