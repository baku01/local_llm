...
class ContentExtractionUtils {
  ...
  static double _calculateContextualScore(
      TextBlock block, List<TextBlock> allBlocks) {
    double contextScore = 0.0;
    final position = block.position;

    // Look at adjacent blocks
    for (int i = math.max(0, position - 2);
        i <= math.min(allBlocks.length - 1, position + 2);
        i++) {
  ...
  static List<ScoredTextBlock> _filterReadableBlocks(
      List<ScoredTextBlock> scoredBlocks) {
    ...
    final avgScore = scores.isNotEmpty
        ? scores.fold<double>(0.0, (sum, value) => sum + value) / scores.length
        : 0.0;
    final threshold = math.max(avgScore * 0.5, 1.0);

    // Filter blocks above threshold
    final filtered =
        scoredBlocks.where((block) => block.score >= threshold).toList();
  ...
  static ContentMetadata _extractContentMetadata(
  ...
  static double _calculateReadabilityScore(String content) {
    ...
    final syllables =
        words.fold<int>(0, (sum, word) => sum + _countSyllables(word));
